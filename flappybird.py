...
# --- SYNTHESIZED SOUND EFFECTS GENERATOR (NO EXTERNAL AUDIO FILES NEEDED) ---
def generate_tone(frequency, duration, volume=0.3, wave_type="sine"):
    ...
    for i in range(n_samples):
        t = i / sample_rate
        if wave_type == "sine":
            val = int(32767 * volume * pygame.math.Vector2(1, 0).rotate(frequency * t * 360).x)
        elif wave_type == "square":
            val = int(32767 * volume) if (i % int(sample_rate / frequency)) < (sample_rate / frequency / 2) else -int(32767 * volume)
...
# Game Sound Effects
try:
    SND_JUMP = generate_tone(400, 0.08, volume=0.2, wave_type="sine")
    SND_SCORE = generate_tone(880, 0.12, volume=0.25, wave_type="sine")
    SND_HIT = generate_tone(120, 0.25, volume=0.3, wave_type="noise")
    SND_CLICK = generate_tone(600, 0.04, volume=0.15, wave_type="square")
    sound_enabled = True
except:
    sound_enabled = False
